...
os.environ.setdefault("BOOTSTRAP_API_KEYS", "test-suite:test-key")
...
class TestAPIReliability:
    ...
    def test_batch_analysis_empty_list(self):
        ...
        response = client.post("/analyze-batch", json=[], headers=AUTH_HEADERS)
        assert response.status_code == 200
    ...
    def test_batch_analysis_too_many(self):
        ...
        contracts = [{"contract_code": "pragma solidity ^0.8.0; contract X {}", "contract_name": f"X{i}"} 
                     for i in range(11)]
        response = client.post("/analyze-batch", json=contracts, headers=AUTH_HEADERS)
        assert response.status_code == 400
