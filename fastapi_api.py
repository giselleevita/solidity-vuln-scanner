...
from api_v1_router import router as api_v1_router
from monitoring import MetricsMiddleware, record_analysis, get_metrics_endpoint, get_health_check
from auth import get_current_user, require_api_key_user
...
logger = setup_logging(
    log_level="INFO" if not get_config().debug else "DEBUG",
    use_json=os.getenv("LOG_FORMAT", "plain").lower() == "json",
)
app_logger = get_logger(__name__)


def init_sentry_stub() -> None:
    """Optional Sentry hook; no hard dependency unless DSN is configured."""
    dsn = os.getenv("SENTRY_DSN", "").strip()
    if not dsn:
        app_logger.info("Sentry disabled (SENTRY_DSN not set)")
        return
    try:
        import sentry_sdk

        sentry_sdk.init(dsn=dsn, traces_sample_rate=0.05)
        app_logger.info("Sentry initialized")
    except Exception as exc:
        app_logger.warning("Sentry initialization skipped: %s", exc)
...
# Mount versioned white-label API router
...
if config.production_mode and "*" in allowed_hosts:
    raise ValueError("ALLOWED_HOSTS must not include '*' in production mode")
...
async def analyze_contract_sarif(
    request: ContractAnalysisRequest,
    current_user: dict = Depends(require_api_key_user),
):
    """
...
async def unregister_webhook(webhook_id: str, current_user: dict = Depends(get_current_user)):
    """Unregister a webhook"""
    success = webhook_manager.unregister_webhook(webhook_id, user_id=current_user.get("user_id"))
    if success:
...
async def list_webhooks(current_user: dict = Depends(get_current_user)):
    ...
    return {
        "webhooks": webhook_manager.list_webhooks(user_id=current_user.get("user_id"))
    }


@app.post("/professional-audit")
async def professional_audit(
    request: ProfessionalAuditRequest,
    current_user: dict = Depends(require_api_key_user),
):
    """
