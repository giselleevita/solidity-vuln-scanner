...
class RateLimiter:
    ...
    def __init__(self, max_requests: Optional[int] = None, window_seconds: Optional[int] = None):
        ...
        self.max_keys = config.rate_limit_max_keys
        self.requests = defaultdict(list)

    def _prune(self, now: float) -> None:
        """Drop expired and excess keys to keep memory usage bounded."""
        window_start = now - self.window_seconds
        stale_keys = [k for k, times in self.requests.items() if not times or times[-1] <= window_start]
        ...
        # Remove least-recently-seen keys first.
        overflow = len(self.requests) - self.max_keys
        for key, _ in sorted(self.requests.items(), key=lambda item: item[1][-1] if item[1] else 0)[:overflow]:
            self.requests.pop(key, None)
