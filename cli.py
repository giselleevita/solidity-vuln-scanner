...
Examples:
  ...
  cat contract.sol | %(prog)s -
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="SEVERITY",
        help="Exit with code 1 if any finding is at or above this severity (CRITICAL > HIGH > MEDIUM > LOW)"
    )
    ...
    if args.fail_on:
        ...
        for vuln in result.get("vulnerabilities", []):
            ...
            if vuln_severity in _SEVERITY_ORDER and _SEVERITY_ORDER.index(vuln_severity) >= threshold_idx:
                if not args.quiet:
                    print(
                        f"\u274c Exiting 1: finding at or above --fail-on threshold ({args.fail_on})",
                        file=sys.stderr,
                    )
                sys.exit(1)
    sys.exit(0)
